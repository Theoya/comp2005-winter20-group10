...
public class RicochetRobots extends JFrame
{
	...
	static Piece yellow;
	static Piece blue;
	static JTextField callBid1 = new JTextField();
    static JTextField callBid2 = new JTextField();
    static JTextField callBid3 = new JTextField();
    static JTextField callBid4 = new JTextField();
	private boolean AITurn = false;
	private String hint = "";
    
    
    public RicochetRobots(Player p_1, Player p_2, Player p_3, Player p_4) 
    {
		...
		redRobot.setBackground(Color.RED);
		JButton yellowRobot = new JButton("yellow");
		yellowRobot.setBackground(Color.YELLOW);
		JButton greenRobot = new JButton("green");
		greenRobot.setBackground(Color.GREEN);
		JButton blueRobot = new JButton("blue");
		blueRobot.setBackground(Color.BLUE);
		JButton hourGlass = new JButton("Hourglass");
		...
		hourGlass.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				JFrame hourglassFrame = new JFrame("Hourglass");
				hourglassFrame.setSize(300,100);
				hourglassFrame.setLocationRelativeTo(null);
				
				JPanel glassPanel = new JPanel();
				glassPanel.setBackground(Color.lightGray);
				JLabel currentBids = new JLabel("Other Bids:");
				JLabel glassCountdown = new JLabel();
				JLabel warn = new JLabel("New bids must be lower than all previous bids");
				...
				timer = new Timer(1000, new ActionListener() {
					public void actionPerformed(ActionEvent evt) {
						...
						glassCountdown.setText("Countdown: " + timeStart);
						timeStart--;
					}
				});
				
				timer.start();
				glassPanel.add(glassCountdown);
				hourglassFrame.setLayout(new GridLayout());
				hourglassFrame.add(glassPanel);
				hourglassFrame.setLocationRelativeTo(null);
				hourglassFrame.setVisible(true);
				}});
		...
		Bid.addActionListener(new ActionListener() {
	        public void actionPerformed(ActionEvent arg0) {
				...
				callBid1.setColumns(4);
	    		callBid2.setColumns(4);
	    		callBid3.setColumns(4);
	    		callBid4.setColumns(4);
	    		JLabel p1 = new JLabel(p_1.getPlayerName());
	    		JLabel p2 = new JLabel(p_2.getPlayerName());
	    		JLabel p3 = new JLabel(p_3.getPlayerName());
				JLabel p4 = new JLabel(p_4.getPlayerName());
				JLabel chip = new JLabel();
	    		
	    		timer = new Timer(1000, new ActionListener() {
	    		    public void actionPerformed(ActionEvent evt) {
	    		        if(timeStart == 0){
	    		        	p1.setText(p_1.getPlayerName() + callBid1.getText()); ////////////////////
	    		        	info.add(p1);
	    		        	p2.setText(p_2.getPlayerName() + callBid2.getText());
	    		            info.add(p2);
	    		            p3.setText(p_3.getPlayerName() + callBid3.getText());
	    		            info.add(p3);
	    		            p4.setText(p_4.getPlayerName() + callBid4.getText());
							info.add(p4);
	    		            ...
	    		            info.validate();
	    		            callBid1.setEnabled(false);
	    		            callBid2.setEnabled(false);
	    		            callBid3.setEnabled(false);
	    		            callBid4.setEnabled(false);
	    		            timer.stop();
	    		...
	    		callBid1.addActionListener(new ActionListener() {
	    		...
	    		callBid1.addActionListener(new ActionListener() {
					...
					@Override
					public void actionPerformed(ActionEvent e) {
						if (Integer.parseInt(callBid1.getText()) <= lowestBid) {
							lowestBid = Integer.parseInt(callBid1.getText());
	    		...
	    		bidPanel.add(callBid1);
	    		bidPanel.add(p2);
	    		bidPanel.add(callBid2);
	    		bidPanel.add(p3);
	    		bidPanel.add(callBid3);
	    		bidPanel.add(p4);
	    		bidPanel.add(callBid4);
	...
	private void moveRobot(String direction){
		...
		int bid = Integer.parseInt(callBid1.getText()) - i;
    ...
    private void incTurn() {
    	...
    	if (t == p_1.getPlayerName()){
    		turn.setText(p_2.getPlayerName());
    	}
    	else if (t == p_2.getPlayerName()){
    		turn.setText(p_3.getPlayerName());
    	}
    	else if (t == p_3.getPlayerName()){
    		turn.setText(p_4.getPlayerName());
    	}
    	else if (t == p_4.getPlayerName()){
    		turn.setText(p_1.getPlayerName());
		}
	...
	public static void simple() {
		...
		buttonList.get(110).setIcon(new ImageIcon("Images/TWall.PNG"));
		...
		prevIcon[0]=(ImageIcon)buttonList.get(93).getIcon();
		prevIcon[1]=(ImageIcon)buttonList.get(179).getIcon();
		prevIcon[2]=(ImageIcon)buttonList.get(58).getIcon();
		prevIcon[3]=(ImageIcon)buttonList.get(0).getIcon();
	...
	private int winConditionCheck(Player player) {
        
        //if all passed in conditions true, n = 1
